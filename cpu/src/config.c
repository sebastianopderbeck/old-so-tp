...
t_cpu_config *create_cpu_config(char *path_a_config)
{
  ...
  cpu_config->quantity_entries_tlb = config_get_string_value(cpu_config->config, "QUANTITY_ENTRIES_TLB");
  cpu_config->tlb_algorithm = config_get_string_value(cpu_config->config, "TLB_ALGORITHM");
  return cpu_config;
}
...
void destroy_cpu_config(t_cpu_config *cpu_config)
{
  ...
  free((void *)cpu_config->quantity_entries_tlb);
  free((void *)cpu_config->tlb_algorithm);
